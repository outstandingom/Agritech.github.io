...
<body>
    <div class="container">
        ...
        <div class="upload-section" id="aiAnalysisSection" style="display: none;">
            ...
            <div class="upload-area" id="uploadArea">
                ...
                <div class="upload-subtext">AI will analyze and verify stage conditions</div>
            ...
            <div class="ai-verification-status" id="aiVerificationStatus">
                <!-- AI verification status will be shown here -->
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const aiVerificationStatus = document.getElementById('aiVerificationStatus');
        ...
        function removeImage() {
            ...
            aiVerificationStatus.style.display = 'none';
        }
        ...
        async function analyzePlant() {
            ...
            // Show loading state
            analyzeButton.innerHTML = '<span class="spinner"></span> AI Analyzing...';
            analyzeButton.disabled = true;
            aiVerificationStatus.innerHTML = '<i class="fas fa-robot"></i> AI is verifying stage conditions...';
            aiVerificationStatus.className = 'ai-verification-status ai-pending';
            aiVerificationStatus.style.display = 'block';
            
            try {
                // Simulate AI analysis with Gemini API
                await new Promise(resolve => setTimeout(resolve, 3000));
                
                // For demo, randomly verify some conditions (simulating AI analysis)
                const stage = plantStages[currentStage];
                const currentStageData = currentCrop.stages[currentStage];
                const currentVerified = currentStageData.ai_verified_conditions || Array(stage.conditions.length).fill(false);
                
                // Find unverified conditions to verify
                const unverifiedIndices = [];
                currentVerified.forEach((verified, index) => {
                    if (!verified) unverifiedIndices.push(index);
                });
                
                // Randomly verify 1-2 conditions (simulating AI detection)
                const conditionsToVerify = Math.min(
                    Math.floor(Math.random() * 2) + 1,
                    unverifiedIndices.length
                );
                
                for (let i = 0; i < conditionsToVerify; i++) {
                    if (unverifiedIndices.length > 0) {
                        const randomIndex = Math.floor(Math.random() * unverifiedIndices.length);
                        const conditionIndex = unverifiedIndices[randomIndex];
                        currentVerified[conditionIndex] = true;
                        unverifiedIndices.splice(randomIndex, 1);
                    }
                }
                
                // Update database with AI verification
                const { error } = await window.supabaseClient
                    .from('crop_stages')
                    .update({ 
                        ai_verified_conditions: currentVerified,
                        last_ai_verification_date: new Date().toISOString(),
                        updated_at: new Date().toISOString()
                    })
                    .eq('crop_id', currentCrop.crop_id)
                    .eq('stage_key', currentStage);
                ...
                if (error) {
                    showError('Database Update Failed', 'Failed to update AI verification in database.', error);
                    return;
                }
                
                // Update local state
                currentCrop.stages[currentStage].ai_verified_conditions = currentVerified;
                
                // Update landCrops
                const landId = userLands[currentLandIndex].land_id;
                const cropIndex = landCrops[landId].findIndex(crop => crop.crop_id === currentCrop.crop_id);
                if (cropIndex !== -1) {
                    landCrops[landId][cropIndex].stages[currentStage].ai_verified_conditions = currentVerified;
                }
                
                // Show success message
                aiVerificationStatus.innerHTML = `<i class="fas fa-check-circle"></i> AI verified ${conditionsToVerify} condition(s)!`;
                aiVerificationStatus.className = 'ai-verification-status ai-verified';
                
                // Update UI
                updateStageConditions();
                
                showNotification(`AI analysis completed! ${conditionsToVerify} condition(s) verified.`);

            } catch (error) {
                showError('AI Analysis Failed', 'Failed to analyze the plant image.', error);
                aiVerificationStatus.style.display = 'none';
            } finally {
                analyzeButton.innerHTML = '<i class="fas fa-search"></i> Analyze with AI';
